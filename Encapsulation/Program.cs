...
namespace Encapsulation
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //!!!Validation:
            //Валидация можем да направим и в конструктор, но обикновено се прави в пропърти (в сетъра), там и е мястото и конструктора вика (сетва) сетъра, а сетъра е прайвет и сетва на прайвет полето (така се преизползва код и така се прави) !!! Дори в случай да нямаме сетер е по добре да си направим прайвет сет и там да сложим валидацията, вместо в конструктора.

            //Информация в нашия клас получаваме през конструктора, пропъртитата и през пъблични полета (ако има такива), като информацията САМО през публично поле не можем да валидираме и затова публични полета НЕ СЕ ИЗПОЛЗВАТ (с тях класа ни става разграден двор) !!!
            ...
            //Когато направим пропърти без сетер това означава че мога само веднъж да сложа ст-ст на това пропърти, или веднага след него с "=" или в конструктора и повече някой включително и аз не може да го променя, правя го ReadOnly и така го енкапсулирам.
            ...
            //readonly се слага само на полета и ако го сложим можем да му дадем ст-ст с "=" срещу него или в конструктора, никаде другаде!!! (игеята е като на пропърти само с гет - пак може само конструктора да му задава ст-ст)
            ...
            //"=>" се използва в съкратения запис на пропъртито или нетода вместо return!!!
            ...
            //if we have private mutable fields are still not encapsulated!!! Когато имаме референтни типове данни моя обект не е Immutable. Immutable е ако благодарение на nякой от интерфейсите IReadOnlyCollection, IReadOnlyDictionary или IReadOnlyList (те изискват съществуването на този метод и според нуждите можем да използваме един от тях) извикам метода .AsReadOnly() и той прави този тип данни такъв, на който е забранено отвън да му се пипат данните!!!
            ...
            //    }
            //}
            // Но с такъв метод вътре в класа аз ще мога да добавям, защото полето си е нормален лист и след като имам достъп от вътре мога (през пропъртито и аз не мога, а то е достъпно отвън)!!!:
            ...
            //Това горе е еквивалентно на това: this.Salary = this.Salary + this.Salary * (percent/100); 
            //}
